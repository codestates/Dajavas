...
function MyPage({type}) {
    ...
    const handleSubmit = async (e) => {
        ...
        try{
            const res = await mypageApi.modifyUserInfo(email, login_method, accessToken, formData);
    ...
    return (
        <>
            MyPage
            {
                ...
                <>
                    <form onSubmit={handleSubmit}>
                        <div>
                            <input name='email' type='text' placeholder='이메일' defaultValue={email} readOnly />
                        </div>
                        <div>
                            <input name='nickname' type='text' placeholder='닉네임' defaultValue={nickname} onChange={handleInputChange}/>
                        </div>
                        <div>
                            <input name='password' type='text' placeholder='비밀번호' defaultValue={password} onChange={handleInputChange}/>
                        </div>

                        <div>
                            <input name='passwordCheck' type='text' placeholder='비밀번호 확인' defaultValue={password} onChange={handleInputChange}/>
                        </div>
                        <div>
                            {errorMessage}
                        </div>
            ...
            <>
                <button onClick={()=>{setIsEditMode(true) && setIsDisabled(false)}}>
                    수정
                </button>
                <button>
                    로그아웃
